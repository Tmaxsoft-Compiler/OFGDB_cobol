...
value_cast (struct type *type, struct value *arg2)
{
  ...
  if (current_language->la_language == language_cobol)
    return value_cast_cobol(type, arg2); /*sylee test*/
...
static struct value *
get_value_at (struct type *type, CORE_ADDR addr, int lazy) /*sylee test*/
{
  struct value *val;
...
int
value_fetch_lazy (struct value *val)  /* sylee test */
{
  ...
  if (length)
    {
	  ...
	  || !traceframe_available_memory (&available_memory, memaddr, length)) /*sylee test*/
	{
	  if (stack)
...
struct value *
value_assign (struct value *toval, struct value *fromval) /* sylee test */
{
  struct type *type;
  struct value *val;
  ...
  if (VALUE_LVAL (toval) != lval_internalvar)
    fromval = value_cast (type, fromval); /*sylee test*/
  else
    {
...
struct value *
value_of_variable (struct symbol *var, const struct block *b) /* sylee test */
{
  struct frame_info *frame;
