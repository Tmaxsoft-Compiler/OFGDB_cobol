		 ...
		 struct ui_file *stream)
{
  ...
  if (VALUE_LVAL (val) == lval_memory) { /*sylee test*/
  ...
  if (options->format == 0 || options->format == 's'
    ...
    value_print (val, stream, options); /* sylee test */
  else
...
print_command_1 (char *exp, int inspect, int voidprint)
{
  ...
  if (voidprint || (val && value_type (val) &&
	...
	printf_filtered ("$%d = ", histindex); /* sylee test */
	...
	annotate_value_history_value ();
      ...
      opts.raw = fmt.raw;
      print_formatted (val, fmt.size, &opts, gdb_stdout); /* sylee test */
      printf_filtered ("\n");
