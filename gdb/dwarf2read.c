  ...
  switch (pdi->tag)
      ...
      else if (pdi->is_external)
	{
	  ...
	  if (pdi->locdesc || pdi->has_type)
	    add_psymbol_to_list (actual_name, strlen (actual_name), /* sylee test */
				 built_actual_name,
...
static void
process_queue (struct objfile *objfile) /* sylee test */
{
...
read_structure_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  if (die->tag == DW_TAG_structure_type)
    ...
    if (die->tag == DW_TAG_class_type) {
    char *message, *saved;
    message = xstrprintf (_("<type code - %d>"), type->main_type->code);
    saved = obstack_copy0 (&cu->objfile->objfile_obstack, message, strlen (message));
    xfree (message);
    type = init_type (TYPE_CODE_ERROR, 0, 0, saved, cu->objfile);
    return type;
    }
...
read_base_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  switch (encoding)
	...
	if (cu->language == language_ada || cu->language == language_m2
	    || cu->language == language_pascal
	    || cu->language == language_fortran
        || cu->language == language_cobol) /* sylee test */
	  code = TYPE_CODE_CHAR;
	type_flags |= TYPE_FLAG_UNSIGNED;
	break;
      case DW_ATE_UTF:
    ...
    if (cu->language == language_cobol) /* sylee test */
      code = TYPE_CODE_DBCS;
	break;
    /* sylee test */
  ...
  // sylee : COBOL attribute 정보를 설정한다 
...
read_subrange_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  else if (cu->language == language_cobol) /*sylee test*/
    {
      low = 1;
    }
...
dump_die_1 (struct ui_file *f, int level, int max_level, struct die_info *die)
{
  ...
  if (die->child != NULL)
	...
	{
      fprintf_unfiltered (f, "[level : %d, max_level : %d]", level, max_level);
	  fprintf_unfiltered (f,
			      " [not printed, max nesting level reached TEST]\n");
	}
    }
